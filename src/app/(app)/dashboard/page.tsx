...
import { DollarSign, Users, FileText, AlertTriangle, TrendingUp, PieChart as PieChartIcon, Zap, FileWarning } from 'lucide-react';
import {
  ChartContainer,
  ChartTooltip,
  ChartTooltipContent,
...
export default function DashboardPage() {
  ...
  const stats = useMemo(() => {
    ...
    // An invoice is Overdue if its status is 'Overdue'.
    // The status is set in InvoiceForm if due date is past and remainingBalance > 0.
    ...
    return (
      <>
        ...
        <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 mb-6">
          {[...Array(5)].map((_, i) => ( // Keep 5 skeletons for the 5 stat cards
            <Card key={i}>
  ...
  return (
    <>
      ...
      />
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 mb-6"> {/* Changed to xl:grid-cols-5 for 5 cards */}
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
