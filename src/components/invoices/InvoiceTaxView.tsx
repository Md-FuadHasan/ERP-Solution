...
import type { Invoice, Customer, CompanyProfile, InvoiceItem, Warehouse } from '@/types';
...
interface InvoiceTaxViewProps {
  ...
  warehouses: Warehouse[]; // Make sure warehouses is passed
}
...
Issue Date: ${format(new Date(invoice.issueDate), 'yyyy-MM-dd')}`;
  ...
  return (
    <div className="p-6 space-y-6 print:p-4 sm:print:p-6 text-foreground bg-background print:bg-transparent print-root-content">
      ...
      <header className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8 print:gap-4 print:mb-4">
        ...
        <div className="md:text-right">
           ...
           <div className="text-sm text-muted-foreground print:text-xs mt-4 md:mt-0">
              ...
              <p>CR: {customer.registrationNumber || '[Customer CR Number]'}</p>
      ...
      <section className="mb-8 print:mb-4">
          ...
          <Table>
            ...
            <TableBody>
              {invoice.items.map((item: InvoiceItem, index: number) => {
                const itemCode = (item as any).itemCode || item.productId || 'N/A'; 
                const discount = (item as any).discount || 0; 
                const amountBeforeDiscount = (item.quantity || 0) * (item.unitPrice || 0);
                const amountAfterDiscount = amountBeforeDiscount - discount;
                const itemVatAmount = amountAfterDiscount * (vatRatePercent / 100);
                const itemTotalWithVAT = amountAfterDiscount + itemVatAmount;
                ...
                return (
                  <TableRow key={item.id || index} className="even:bg-muted/20 print:even:bg-muted/50">
                    ...
                    <TableCell className="text-center text-muted-foreground py-2 print:py-1 text-xs print:text-2xs">{item.quantity} ({item.unitType})</TableCell>
                    <TableCell className="text-right text-muted-foreground py-2 print:py-1 text-xs print:text-2xs">${amountBeforeDiscount.toFixed(2)}</TableCell>
                    <TableCell className="text-right text-destructive py-2 print:py-1 text-xs print:text-2xs">${discount.toFixed(2)}</TableCell>
                    <TableCell className="text-right text-muted-foreground py-2 print:py-1 text-xs print:text-2xs">${itemVatAmount.toFixed(2)}</TableCell>
                    <TableCell className="text-right font-medium text-foreground py-2 pr-4 sm:pr-6 print:pr-2 print:py-1 text-xs print:text-2xs">${itemTotalWithVAT.toFixed(2)}</TableCell>
                  </TableRow>
                );
              })}
            </TableBody>
       ...
       <section className="flex flex-col-reverse md:flex-row justify-between items-start mb-8 gap-8 print:mb-4 print:gap-4">
           <div className="w-full md:w-auto flex flex-col items-center md:items-start print:hidden">
                <h3 className="text-sm font-medium text-muted-foreground mb-2 flex items-center"><QrCode className="w-4 h-4 mr-2"/>Scan for Details</h3>
                <div className="p-2 border rounded-lg inline-block bg-white shadow-sm">
                    <QRCodeCanvas value={qrCodeValue} size={100} bgColor="#ffffff" fgColor="#000000" level="Q" />
                </div>
            </div>
            <div className="w-full md:max-w-sm space-y-2.5 text-sm border p-4 sm:p-6 rounded-lg bg-muted/40 print:p-3 print:space-y-1.5">
              <div className="flex justify-between">
                <span className="text-muted-foreground">Sub-Total (incl. Item Excise):</span>
                <span className="font-medium text-foreground">${(typeof invoice.subtotal === 'number' ? invoice.subtotal : 0).toFixed(2)}</span>
              </div>
                <div className="flex justify-between">
                    <span className="text-muted-foreground">Total Discount:</span>
                    <span className="font-medium text-destructive">-${(invoice.items.reduce((sum, item) => sum + ((item as any).discount || 0), 0)).toFixed(2)}</span>
                </div>
              {invoice.vatAmount > 0 && (
                <div className="flex justify-between">
              ...
              <Separator className="my-3 !bg-border print:my-1" />
              <div className="flex justify-between text-base font-semibold print:text-sm">
                <span className="text-foreground">Total Billing Amount:</span>
                <span className="text-foreground">${(typeof invoice.totalAmount === 'number' ? invoice.totalAmount : 0).toFixed(2)}</span>
              </div>
               <div className="flex justify-between text-green-600 dark:text-green-400 print:text-sm">
              ...
              <div className="flex justify-between text-lg font-bold print:text-base">
                ...
                <span className={`${(typeof invoice.remainingBalance === 'number' ? invoice.remainingBalance : 0) > 0 ? 'text-destructive' : 'text-green-600 dark:text-green-400'}`}>
      ...
      <section className="mb-8 print:mb-4 text-sm text-muted-foreground print:text-xs">
        <h3 className="text-base font-semibold mb-2 text-foreground print:text-sm print:mb-1">Terms & Conditions:</h3>
        <ol className="list-decimal list-inside space-y-1">
            <li>Payment is due within {customer.invoiceAgingDays || 30} days from the invoice date.</li>
            <li>Please include the invoice number on your payment.</li>
            <li>A late fee of 1.5% per month may be charged on overdue amounts.</li>
        </ol>
      </section>

       {/* Footer Message */}
        <div className="mt-12 text-center text-xs text-muted-foreground print:mt-6">
            <p>Thank you for your business!</p>
            {companyProfile && <p>If you have any questions concerning this invoice, please contact {companyProfile.name} at {companyProfile.phone} or {companyProfile.email}.</p>}
        </div>
    </div>
  );
};
